...
@Slf4j // log 객체 자동 생성
public class Ch8_RewriteQueryTransformerService {

    private final Advisor retrievalAugmentationAdvisor; // 질문 재작성 + 문서 검색 + 문맥 보강이 적용된 RAG Advisor
    private final ChatClient chatClient; // Logger Advisor 가 적용된 ChatClient

    //Constructor
    public Ch8_RewriteQueryTransformerService(ChatClient.Builder chatClientBuilder, VectorStore vectorStore) { // ChatClient.Builder 와 VectorStore 를 주입받는 생성자
        ...
        retrievalAugmentationAdvisor = RetrievalAugmentationAdvisor.builder() // Retrieval Augmentation Advisor 생성 시작
                .queryTransformers(RewriteQueryTransformer.builder() // 사용자 질문을 더 검색하기 좋은 형태로 다시 작성하는 QueryTransformer 생성 시작
                        .chatClientBuilder(chatClientBuilder) // 내부적으로 질문 재작성을 수행할 ChatClient 생성에 사용할 Builder 전달
                        .build()) // RewriteQueryTransformer 생성 완료
    ...
    public Flux<String> ragChat(String question, String type, String conversationId) { // 질문을 재작성한 뒤 type 조건에 맞는 문서를 검색해서 답변하는 메서드

        return this.chatClient.prompt() // 새 프롬프트 생성 시작
                .system("친절하게 한국어로 답변해줘.") // 시스템 메시지 설정
                .user(question) // 사용자 질문 설정
                .advisors(retrievalAugmentationAdvisor) // 질문 재작성 + 문서 검색 + 문맥 보강 Advisor 적용
