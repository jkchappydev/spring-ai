...
@Slf4j // log 객체 자동 생성
public class Ch8_MultiQueryExpanderService {

    private final Advisor retrievalAugmentationAdvisor; // 다중 질의 확장 + 문서 검색 + 문맥 보강이 적용된 RAG Advisor
    private final ChatClient chatClient; // Logger Advisor 가 적용된 ChatClient

    //Constructor
    public Ch8_MultiQueryExpanderService(ChatClient.Builder chatClientBuilder, VectorStore vectorStore) { // ChatClient.Builder 와 VectorStore 를 주입받는 생성자
        ...
        retrievalAugmentationAdvisor = RetrievalAugmentationAdvisor.builder() // Retrieval Augmentation Advisor 생성 시작
                .queryExpander(MultiQueryExpander.builder() // 하나의 질문을 여러 개의 검색 질의로 확장하는 QueryExpander 생성 시작
                        .chatClientBuilder(chatClientBuilder) // 내부적으로 질의 확장을 수행할 ChatClient 생성에 사용할 Builder 전달
                        // 사용자가 입력한 Question을 같이 사용할지 결정
                        //.includeOriginal(false) // 원본 질문을 확장 질의 목록에 포함할지 설정 가능
                        .numberOfQueries(3) // 원본 질문을 바탕으로 3개의 확장 질의 생성
                        .build()) // MultiQueryExpander 생성 완료
    ...
    public Flux<String> ragChat(String question, String type, String conversationId) { // 질문을 여러 개의 검색 질의로 확장한 뒤 type 조건에 맞는 문서를 검색해서 답변하는 메서드

        return this.chatClient.prompt() // 새 프롬프트 생성 시작
                .system("친절하게 한국어로 답변해줘.") // 시스템 메시지 설정
                .user(question) // 사용자 질문 설정
                .advisors(retrievalAugmentationAdvisor) // 다중 질의 확장 + 문서 검색 + 문맥 보강 Advisor 적용
