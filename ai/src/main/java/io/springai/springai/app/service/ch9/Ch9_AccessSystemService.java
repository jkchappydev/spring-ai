...
@Service
@Slf4j
public class Ch9_AccessSystemService {

    private final ChatClient chatClient; // ToolCallbackProvider 가 적용된 ChatClient
    ...
    // Constructor
    public Ch9_AccessSystemService(ChatClient.Builder chatClientBuilder, ToolCallbackProvider toolCallbackProvider) { // ChatClient.Builder 와 ToolCallbackProvider 를 주입받는 생성자
        this.chatClient = chatClientBuilder
                .defaultToolCallbacks(toolCallbackProvider) // 애플리케이션에 등록된 ToolCallback 들을 기본 Tool 로 사용하도록 설정
                .build(); // ChatClient 생성
    }
    ...
    public String imageAnalysisText(String contentType, byte[] bytes) { // 전달받은 이미지에서 사번을 판별하고 출입 가능 여부를 처리하는 메서드
        ...
        return chatClient.prompt() // 새 프롬프트 생성 시작
                ...
                //.tools(new AccessSystemTools()) // 필요 시 개별 Tool 을 직접 지정할 수 있음, 이후 MCP Server 연동 형태로 확장 가능
                //.tools(new IdCardTools()) // 필요 시 사번 판별 Tool 도 개별 등록 가능
                .call() // 모델 호출 수행
                .content(); // 응답 본문만 추출
    }
